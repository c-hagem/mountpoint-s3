...
def mount_mp(cfg: DictConfig, mount_dir: str) -> Dict[str, Any]:
    ...
    if mp_config["mountpoint_clone_fuse_fd"] is not None:
        mp_env["MOUNTPOINT_CLONE_FUSE_FD"] = str(mp_config["mountpoint_clone_fuse_fd"])

    if mp_config["mountpoint_jitter"] is not None:
        mp_env["MOUNTPOINT_UNSTABLE_JITTER"] = str(mp_config["mountpoint_jitter"])
